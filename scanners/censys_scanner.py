...
class CensysScanner(BaseScanner):
    ...
    def _get_client(self):
        ...
        if self._client is None:
            try:
                ...
                logger.warning("censys package not installed. Install with: pip install censys")
                self.enabled = False
                raise ImportError("censys package not available")
    ...
    def scan(self) -> ScanResult:
        ...
        try:
            ...
            logger.warning("Censys scanner disabled: censys package not installed")
        except Exception as e:
...
class CensysCertScanner(BaseScanner):
    ...
    def _get_client(self):
        if self._client is None:
            try:
                ...
                logger.error(f"Failed to initialize Censys cert client: {e}")
                self.enabled = False
                raise
        return self._client
    ...
    def scan(self) -> ScanResult:
        ...
        try:
            ...
            logger.warning("Censys cert scanner disabled: censys package not installed")
        except Exception as e:
            logger.error(f"Censys cert scan failed: {e}")
